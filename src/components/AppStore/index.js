...
class AppStore extends Component {
  ...
  getSearchInputApps = name => {
    const {searchInput} = this.state
    const searchApps = name.filter(eachSearch =>
      eachSearch.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchApps
  }
  ...
  render() {
    const {category} = this.state
    const {searchInput} = this.state
    const filterApps = this.getFilterTheAppsList()
    const searchResults = this.getSearchInputApps(filterApps)
